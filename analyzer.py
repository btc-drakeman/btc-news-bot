...
def analyze_symbol(symbol: str):
    try:
        ...
        if not data or '15m' not in data or '30m' not in data or '1m' not in data:
            print(f"❌ 데이터 부족 또는 필요한 봉 미존재: {symbol}")
            return None
...
"""
        ...
        print(f"❌ [analyze_symbol 전체 실패] {symbol}: {e}")
        return None
